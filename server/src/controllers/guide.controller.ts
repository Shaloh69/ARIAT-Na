...
export const getCuratedGuides = async (
  req: Request,
  res: Response,
): Promise<void> => {
  ...
  const guides = (rows as any[]).map((g) => ({
    ...
    clusters:
      typeof g.clusters === "string"
        ? JSON.parse(g.clusters)
        : (g.clusters ?? []),
    interests:
      typeof g.interests === "string"
        ? JSON.parse(g.interests)
        : (g.interests ?? []),
    destination_ids:
      typeof g.destination_ids === "string"
        ? JSON.parse(g.destination_ids)
        : (g.destination_ids ?? []),
  }));
