 ...
 * }
 */
export const generateKioskItinerary = async (req: Request, res: Response): Promise<void> => {
  const {
    start_lat,
    start_lon,
    interests = [],
    ...
    try {
      ...
      await pool.execute(
        `INSERT INTO kiosk_sessions (id, token, itinerary_data, days, transport_mode, is_claimed, expires_at)
         VALUES (?, ?, ?, ?, ?, FALSE, ?)`,
        [uuidv4(), token, JSON.stringify(itineraryData), numDays, transport_mode, expiresAt],
      );
      break;
    } catch (err: any) {
  ...
  const totalStops = numDays > 1
    ? ((itineraryData as any).days ?? []).reduce((s: number, d: any) => s + (d.itinerary?.stops?.length ?? 0), 0)
 ...
 */
export const previewKioskSession = async (req: Request, res: Response): Promise<void> => {
  ...
  if ((rows as any[]).length === 0) {
    throw new AppError('Session not found or expired. Please generate a new QR at the kiosk.', 404);
  }

  const session = (rows as any[])[0];
  let itinerary: object;
  try {
    itinerary = typeof session.itinerary_data === 'string'
 ...
 */
export const claimKioskSession = async (req: AuthRequest, res: Response): Promise<void> => {
  const { token } = req.params;
  if (!token) throw new AppError('Token is required', 400);
  ...
  if ((rows as any[]).length === 0) {
    throw new AppError('Session not found or expired. Please scan a fresh QR code.', 404);
  }
  ...
  // Build title
  const totalStops = numDays > 1
    ? (itinerary.days ?? []).reduce((s: number, d: any) => s + (d.itinerary?.stops?.length ?? 0), 0)
  ...
  await pool.execute(
    ...
    [
      ...
      numDays > 1
        ? (itinerary.days ?? []).reduce((s: number, d: any) => s + (d.itinerary?.totalDistance ?? 0), 0)
        : (itinerary.totalDistance ?? 0),
      numDays > 1
        ? (itinerary.days ?? []).reduce((s: number, d: any) => s + (d.itinerary?.estimatedTotalTime ?? 0), 0)
        : (itinerary.estimatedTotalTime ?? 0),
      numDays > 1
        ? (itinerary.days ?? []).reduce((s: number, d: any) => s + (d.itinerary?.estimatedCost ?? 0), 0)
  ...
  type StopRow = { destination?: any; visit_duration?: number; cumulative_time?: number; leg_distance?: number; leg_travel_time?: number };
  const insertStop = async (stop: StopRow, dayNumber: number, visitOrder: number) => {
  ...
  logger.info(`[KIOSK] Session ${token} claimed by user ${userId} → itinerary ${itineraryId}`);
