...
export const deactivateAdmin = async (req: AuthRequest, res: Response): Promise<void> => {
  if (req.user?.role !== 'super_admin') {
    throw new AppError('Only super admins can deactivate accounts', 403);
  }

  const { id } = req.params;
  if (id === req.user.id) throw new AppError('You cannot deactivate your own account', 400);

  const [rows] = await pool.execute<RowDataPacket[]>(
    'SELECT id, role, is_active FROM admins WHERE id = ?', [id]
  );
  ...
  if (!target.is_active) throw new AppError('Account is already deactivated', 400);
...
export const reactivateAdmin = async (req: AuthRequest, res: Response): Promise<void> => {
  if (req.user?.role !== 'super_admin') {
  ...
  if ((rows as any[]).length === 0) throw new AppError('Admin not found', 404);
  if ((rows as any[])[0].is_active) throw new AppError('Account is already active', 400);
...
export const getChatHistory = async (_req: AuthRequest, res: Response): Promise<void> => {
  const [rows] = await pool.execute<RowDataPacket[]>(
