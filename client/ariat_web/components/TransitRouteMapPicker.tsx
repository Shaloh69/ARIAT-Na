...
export default function TransitRouteMapPicker({
  ...
  const roadLayersRef = useRef<Map<string, { line: L.Polyline; decorator?: any }>>(new Map());
  ...
  useEffect(() => {
    ...
    roadLayersRef.current.forEach((layers, id) => {
      if (!roads.find(r => r.id === id)) {
        ...
        if (layers.decorator) try { map.removeLayer(layers.decorator); } catch { /* ignore */ }
    ...
    roads.forEach((road) => {
      ...
      if (existing) {
        ...
        if (existing.decorator) {
          try { map.removeLayer(existing.decorator); } catch { /* ignore */ }
          existing.decorator = undefined;
        }
      } else {
        const line = L.polyline(road.positions, { color, weight, opacity, interactive: true });
        line.addTo(map);
      ...
      if (isSelected && !road.isBidirectional && (window as any).L?.polylineDecorator) {
        const entry = roadLayersRef.current.get(road.id);
        if (entry && !entry.decorator) {
          try {
            const dec = (L as any).polylineDecorator(road.positions, {
              patterns: [{
                offset: "50%", repeat: 0,
                symbol: (L as any).Symbol.arrowHead({
                  pixelSize: 12, polygon: false,
                  pathOptions: { stroke: true, color: routeColor, weight: 2, opacity: 0.9 },
                }),
              }],
            }).addTo(map);
            entry.decorator = dec;
          } catch { /* polylineDecorator may not be loaded */ }
        }
      }
    });
      ...
      if (existing) {
        ...
        if (pickupMode === "stops_only") {
          marker.on("click", () => {
            ...
            if (cur.has(stop.id)) { cur.delete(stop.id); } else { cur.add(stop.id); }
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div
        ...
        style={{ background: "rgba(2,6,23,0.88)", border: "1px solid rgba(255,255,255,0.12)", color: "#e2e8f0" }}
      >
        ...
        <p className="text-xs pt-1" style={{ color: "#94a3b8" }}>
          {selectedRoadIds.length} road{selectedRoadIds.length !== 1 ? "s" : ""} selected
          {pickupMode === "stops_only" && ` · ${selectedStopIds.length} stop${selectedStopIds.length !== 1 ? "s" : ""}`}
        </p>
      </div>
    </div>
  );
